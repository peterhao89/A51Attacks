...
public:
	...
	void constructEquations(u64 moves, vector<u64> clockBits, u64 prefix, int moveNumber) {
		for (int i = 0; i < moveNumber; ++i) {
			u64 localClock = 0;
			for (auto bt = 0; bt < 3; ++bt) {
				setBitVal(localClock, bt, bitW64(clockBits, 3 * i + bt));
			}
			doOneMove((moves >> (2 * i)) & 0x3,localClock, bit64(prefix, i));
		}
	}
	...
	void doOneMove(u64 oneMoveMask, u64 oneClock, u64 prefixBit) {
		vector<u64> clkBitAnfs = {
			stateTrack.getRiClockBitEquation(1),
			stateTrack.getRiClockBitEquation(2),
			stateTrack.getRiClockBitEquation(3)
		};
		setOneEquation(clkBitAnfs[0], bit64(oneClock,0));
		setOneEquation(clkBitAnfs[1], bit64(oneClock, 1));
		setOneEquation(clkBitAnfs[2], bit64(oneClock, 2));
		u64 k1 = (known & MASK64R1);
		u64 k2 = (known & MASK64R2);
		u64 k3 = (known & MASK64R3);
		switch (oneMoveMask)
